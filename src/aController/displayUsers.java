...
public class displayUsers implements Initializable {
    ...
    public void updateData() {
        ...
        try {
            ...
            // Handle parsing exception by setting dobValue to a default value
        ...
        result.ifPresent(updatedUser -> {
            try {
                // Validation: Check if the type is valid (faculty, admin, or student)
                String updatedType = updatedUser.getTypee().toLowerCase();
                if (!updatedType.equals("faculty") && !updatedType.equals("admin") && !updatedType.equals("student")) {
                    showAlert(Alert.AlertType.ERROR, "Invalid Type", "The type must be one of the following: 'faculty', 'admin', or 'student'.");
                    return;
                }
                ...
                String query = String.format(
                    "UPDATE users SET FName='%s', LName='%s', type='%s', dob='%s' WHERE id=%d",
                    updatedUser.getFname(),
                    updatedUser.getLname(),
                    updatedUser.getTypee(),
                    updatedUser.getDob(),
                    updatedUser.getId()
                );
                ...
                showAlert(Alert.AlertType.ERROR, "Database Error", "Error updating user. Please try again.\n" + e.getMessage());
            }
        });
    }
